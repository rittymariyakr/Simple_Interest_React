...
function App() {
  ...
  const getValidate = (e)=>{
    ...
    // console.log(!!value.match(/^[0-9]+$/)); //to get true or false 
    if(!!value.match(/^[0-9]*.?[0-9]+$/)){  //to convert boolean two !! marks used
      if(name==='principle'){
        setPrinciple(value)
        setIsPrinciple(true)
      }
...
const handleCalculate = (e)=>{
  e.preventDefault()  //to prevent unwanted reloading
    ...
    <div style={{height:'100vh'}} className='d-flex justify-content-center align-items-center w-100 bg-dark'>
      <div className='bg-light p-5 rounded' style={{width:'500px'}}>
        ...
        <form className='mt-5' onSubmit={handleCalculate}>
            <div className='mb-3'> 
            {/*if there is no input value then display label so we use || symbol */}
